...
public class RegistroController {
    ...
    private void abrirInicioSesion() {
        System.out.println("Abrir ventana de inicio de sesion...");
        try {
            Node inicioSesionContent = FXMLLoader.load(getClass().getResource("/org/fran/chatoffline/ui/inicioSesion.fxml"));
            // Obtener la escena actual y reemplazar el contenido
            Stage stage = (Stage) topBar.getScene().getWindow();
            stage.getScene().setRoot((Parent) inicioSesionContent);
        } catch (IOException e) {
            e.printStackTrace();
            mostrarAlerta("Error al cargar la ventana de inicio de sesion.");
        }
